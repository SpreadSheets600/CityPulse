  ...
  <div class="min-h-screen bg-base-100 text-base-content antialiased p-6">
    <div class="max-w-7xl mx-auto">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <h1 class="text-3xl font-extrabold text-slate-100 font-mono tracking-wider uppercase">Admin Dashboard</h1>
        <div class="text-xs font-mono text-slate-500">SYSTEM STATUS: <span class="text-emerald-400 font-bold">ONLINE</span></div>
      </div>

      <!-- Admin Actions / Navbar Strip -->
      <div class="flex flex-wrap gap-3 mb-8 bg-base-200 border border-base-300 p-4 rounded-3xl">
        <router-link to="/admin/analytics" class="btn btn-outline border-base-300 hover:border-slate-500 hover:bg-base-300 rounded-xl btn-sm font-mono tracking-wide">
        ...
        <router-link to="/admin/audit-log" class="btn btn-outline border-base-300 hover:border-slate-500 hover:bg-base-300 rounded-xl btn-sm font-mono tracking-wide">
          <svg class="w-4 h-4 mr-2 text-secondary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
          Audit Log
        </router-link>
        <router-link to="/admin/departments" class="btn btn-outline border-base-300 hover:border-slate-500 hover:bg-base-300 rounded-xl btn-sm font-mono tracking-wide">
          <svg class="w-4 h-4 mr-2 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
          Departments
        </router-link>
        <router-link to="/admin/sla" class="btn btn-outline border-base-300 hover:border-slate-500 hover:bg-base-300 rounded-xl btn-sm font-mono tracking-wide">
          <svg class="w-4 h-4 mr-2 text-warning" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          SLA Tracking
        </router-link>
        <router-link to="/admin/geofence" class="btn btn-outline border-base-300 hover:border-slate-500 hover:bg-base-300 rounded-xl btn-sm font-mono tracking-wide">
          <svg class="w-4 h-4 mr-2 text-info" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><circle cx="12" cy="11" r="3" /></svg>
          Geofencing
        </router-link>
        <router-link to="/admin/export" class="btn btn-primary rounded-xl btn-sm font-mono tracking-wide ml-auto">
          <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
          Export Reports
        </router-link>
      </div>
      ...
      <div v-if="error" class="bg-red-500/10 border border-red-500/20 text-red-400 rounded-2xl p-5 mb-6 font-mono text-sm">
        <div class="flex">
      ...
      <!-- Main Content -->
      <div v-else-if="issues.length === 0" class="text-center py-16 border border-dashed border-base-300 rounded-3xl bg-base-200/40">
        ...
        <p class="mt-1.5 text-sm text-slate-500">Wait for users in neighborhood geofences to submit reports.</p>
      </div>
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <!-- Total -->
          <div class="bg-base-200 border border-base-300 shadow-lg rounded-2xl p-5 hover:border-primary/40 transition-all duration-300 flex items-center">
            ...
            <div class="ml-4 flex-1">
              ...
              <p class="text-2xl font-extrabold text-slate-100 font-mono mt-0.5">{{ issues.length }}</p>
            </div>
          </div>
          ...
          <div class="bg-base-200 border border-base-300 shadow-lg rounded-2xl p-5 hover:border-primary/40 transition-all duration-300 flex items-center">
            ...
            <div class="ml-4 flex-1">
              ...
              <p class="text-2xl font-extrabold text-slate-100 font-mono mt-0.5">{{ getStatusCount('pending') }}</p>
            </div>
          </div>
          ...
          <div class="bg-base-200 border border-base-300 shadow-lg rounded-2xl p-5 hover:border-primary/40 transition-all duration-300 flex items-center">
            ...
            <div class="ml-4 flex-1">
              ...
              <p class="text-2xl font-extrabold text-slate-100 font-mono mt-0.5">{{ getStatusCount('in_progress') }}</p>
            </div>
          </div>
          ...
          <div class="bg-base-200 border border-base-300 shadow-lg rounded-2xl p-5 hover:border-primary/40 transition-all duration-300 flex items-center">
            ...
            <div class="ml-4 flex-1">
              ...
              <p class="text-2xl font-extrabold text-slate-100 font-mono mt-0.5">{{ getStatusCount('resolved') }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-base-200 border border-base-300 shadow-lg rounded-3xl p-6 mb-8">
          <div class="flex items-center gap-2 mb-4">
            ...
            <h3 class="font-bold text-slate-100 font-mono text-sm tracking-wider uppercase">Live Incident Map Preview</h3>
          </div>
          <div class="map-container h-80 sm:h-[450px] border border-base-300 rounded-2xl overflow-hidden">
          ...
          <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6 gap-4">
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <button @click="selectedStatus = 'rejected'"
                :class="[selectedStatus === 'rejected' ? 'bg-red-500/20 text-red-400 border-red-500/30' : 'bg-base-100 text-slate-400 border-base-300 hover:text-slate-300', 'px-3 py-1.5 text-xs font-mono border rounded-xl transition-all duration-200 cursor-pointer']">
                REJECTED
              </button>
              <button @click="selectedStatus = 'verified'"
                :class="[selectedStatus === 'verified' ? 'bg-purple-500/20 text-purple-400 border-purple-500/30' : 'bg-base-100 text-slate-400 border-base-300 hover:text-slate-300', 'px-3 py-1.5 text-xs font-mono border rounded-xl transition-all duration-200 cursor-pointer']">
                VERIFIED
              </button>
            </div>
          </div>
              ...
              <router-link :to="{ name: 'AdminIssueManage', params: { id: issue.id } }"
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                  <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1 line-clamp-2 leading-relaxed">{{ issue.description }}</p>
                  </div>
