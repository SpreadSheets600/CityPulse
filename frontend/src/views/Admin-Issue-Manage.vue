  ...
  <div class="min-h-screen bg-base-100 text-base-content antialiased py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-5xl mx-auto">
      <div class="mb-8 flex items-center justify-between gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-100 font-mono tracking-wider uppercase">Manage Issue</h1>
          <p class="text-xs text-slate-400 mt-1">Update dispatch status and run AI verification diagnostics</p>
        </div>
        <router-link to="/admin-dashboard" class="btn btn-outline border-base-300 hover:border-slate-500 rounded-xl btn-sm font-mono cursor-pointer">
          ← Back
        </router-link>
      </div>
      ...
      <div v-else-if="error" class="border border-error/20 bg-error/5 text-error text-center text-xs font-mono p-4 rounded-xl mb-6">{{ error }}</div>
      ...
      <div v-else-if="issue" class="space-y-6">
        ...
        <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 text-xs font-mono text-slate-400 border-t border-base-300/50 pt-5 mb-5">
            <div class="flex items-center">
              <svg class="w-4 h-4 mr-2 text-primary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
              </svg>
              <span>Type : <strong class="text-slate-200 ml-1">{{ issue.issue_type }}</strong></span>
            </div>
            <div class="flex items-center">
              <svg class="w-4 h-4 mr-2 text-secondary" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span>Created : <strong class="text-slate-200 ml-1">{{ new Date(issue.created_at).toLocaleDateString() }}</strong></span>
            </div>
            <div v-if="issue.updated_at !== issue.created_at" class="flex items-center">
              ...
              <span>Updated : <strong class="text-slate-200 ml-1">{{ new Date(issue.updated_at).toLocaleDateString() }}</strong></span>
            </div>
            <div class="flex items-center">
              <svg class="w-4 h-4 mr-2 text-accent" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round"
                  d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              <span>Reporter : <strong class="text-slate-200 ml-1">{{ issue.user?.firstname }} {{ issue.user?.lastname }}</strong></span>
            </div>
          </div>
          ...
          <div class="flex items-center text-xs font-mono text-slate-400 mb-6">
            ...
            <span>Address : <strong class="text-slate-200 ml-1">{{ issue.address }}</strong></span>
          </div>

          <!-- Location Map -->
          <div v-if="issue.latitude && issue.longitude" class="pt-4 border-t border-base-300/40">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Geographic Location</h3>
          ...
          <!-- Media Section -->
          <div v-if="issue.image_urls?.length > 0 || issue.video_note_url || issue.voice_note_url" class="pt-6 mt-6 border-t border-base-300/40">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Media Evidence</h3>
            <div class="space-y-6">
              <div v-if="issue.image_urls && issue.image_urls.length > 0">
                <h4 class="text-xs font-mono font-bold text-slate-500 mb-3 flex items-center uppercase tracking-wider">
              ...
              <div v-if="issue.video_note_url">
                ...
                <video controls :src="issue.video_note_url" class="w-full max-w-md rounded-2xl border border-base-300 bg-black"></video>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <!-- Update status -->
          <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
            <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest mb-4">Update Status</h3>
            <select v-model="status" class="select select-bordered w-full rounded-xl border-base-300 font-mono text-xs focus:border-primary">
            ...
            <button @click="saveStatus" class="btn btn-primary mt-4 w-full rounded-xl font-bold cursor-pointer">Save Status</button>
          </div>

          <!-- Assign department -->
          <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
            <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest mb-4">Assign Department</h3>
            <p v-if="issue.department" class="mb-3 text-xs font-mono text-slate-400">Current Department: <span
                class="font-bold text-slate-200">{{ issue.department.name }}</span></p>
            <select v-model="departmentId" class="select select-bordered w-full rounded-xl border-base-300 font-mono text-xs focus:border-primary">
              <option disabled value="">Select Department</option>
              <option v-for="d in departments" :key="d.id" :value="d.id">{{ d.name }}</option>
            </select>
            <button @click="assignDepartment" class="btn btn-accent mt-4 w-full rounded-xl font-bold cursor-pointer">Assign Department</button>
          </div>
        </div>
        ...
        <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
          <div class="flex items-center justify-between mb-4 gap-2">
            <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest flex items-center gap-2">
          ...
          <div v-if="!verification && !verifying" class="text-xs font-mono text-slate-500 py-4">
            Trigger visual inspection. Locally hosted AI models will parse the coordinates and match objects inside the attachment payload.
          </div>

          <div v-if="verification" class="space-y-4">
            <!-- Verification Status Banner -->
            <div :class="{
              'bg-emerald-500/10 border-emerald-500/20 text-emerald-400': verification.status === 'consistent',
              'bg-red-500/10 border-red-500/20 text-red-400': verification.status === 'misleading',
              'bg-yellow-500/10 border-yellow-500/20 text-yellow-400': verification.status === 'uncertain',
            }" class="border rounded-2xl p-4 font-mono text-xs leading-relaxed">
              <div class="flex items-start gap-3">
                <div class="flex-shrink-0 mt-0.5">
                  <svg v-if="verification.status === 'consistent'" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <svg v-else-if="verification.status === 'misleading'" class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div>
                  ...
                  <div class="mt-2 flex flex-wrap gap-4 text-slate-400">
                    <span>REPORTED_TYPE: <strong>{{ verification.reported_type }}</strong></span>
                    <span>AI_DETECTION: <strong>{{ verification.ai_category }}</strong></span>
            ...
            <div v-if="detections.length > 0">
              <h4 class="text-xs font-mono font-bold text-slate-500 uppercase tracking-wider mb-3">Visual Inspection Details</h4>
              <div class="space-y-3">
                <div v-for="det in detections" :key="det.image_index"
                  ...
                  <div class="flex items-center justify-between mb-2">
                    <span class="font-bold text-slate-300">File Object {{ det.image_index + 1 }}</span>
                    <span :class="{
                      'badge badge-success badge-sm': det.category !== 'Unspecified' && det.category !== 'error',
                      'badge badge-warning badge-sm': det.category === 'Unspecified',
                      'badge badge-error badge-sm': det.category === 'error',
                    }" class="font-bold uppercase tracking-wider">{{ det.category }}</span>
                  </div>
        ...
        <!-- Post update -->
        <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
          <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest mb-4">Post SLA Milestone Update</h3>
          <input v-model="updateTitle" type="text" placeholder="Update milestone title" class="input input-bordered w-full rounded-xl border-base-300 focus:border-primary font-sans text-sm" />
          <textarea v-model="updateBody" rows="4" placeholder="Update milestone details..."
            class="textarea textarea-bordered mt-3 w-full rounded-xl border-base-300 focus:border-primary font-sans"></textarea>
          
          <div class="my-4">
            <label class="block text-xs font-mono text-slate-400 uppercase tracking-wider mb-2">SLA Task Progress : {{ progress }}%</label>
            <input v-model.number="progress" type="range" min="0" max="100" class="range range-primary" />
          </div>
          
          <input type="file" multiple accept="image/*" @change="onUpdateFileChange" class="file-input file-input-bordered file-input-primary w-full rounded-xl" />
          <button @click="postUpdate" class="btn btn-primary mt-4 w-full rounded-xl font-bold cursor-pointer shadow-lg shadow-blue-500/10">PUBLISH SLA UPDATE</button>
        </div>

        <!-- Existing updates -->
        <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl p-6 md:p-8">
          <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest mb-4">Past Milestones</h3>
          <div v-if="updates.length === 0" class="text-sm font-mono text-slate-500 py-4">No update files posted.</div>
          <div v-else class="space-y-4">
            <div v-for="u in updates" :key="u.id" class="bg-base-100/50 border border-base-300 rounded-2xl p-5 shadow-inner">
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                ...
                <span class="text-xs font-mono text-slate-500">{{ new Date(u.created_at).toLocaleString() }}</span>
              </div>
              ...
              <div class="mb-3">
                ...
                <div class="w-full bg-base-300 rounded-full h-2">
                  <div class="bg-primary h-2 rounded-full transition-all duration-500" :style="{ width: (u.progress || 0) + '%' }"></div>
                </div>
              </div>
