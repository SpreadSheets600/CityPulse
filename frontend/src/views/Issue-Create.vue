  ...
  <div class="min-h-screen bg-base-100 text-base-content antialiased py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="flex justify-between items-center mb-8 gap-4">
        <div>
          <h1 class="text-3xl font-extrabold text-slate-100 font-mono tracking-wider uppercase">Report New Issue</h1>
          <p class="text-xs text-slate-400 mt-1">Submit geolocated media evidence for verification</p>
        </div>
      ...
      <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl overflow-hidden mb-6">
        <form @submit.prevent="submitIssue" class="divide-y divide-base-300 flex flex-col">
          ...
          <div class="p-6 md:p-8">
            <h2 class="text-lg font-bold font-mono text-slate-200 mb-6 uppercase tracking-wider text-xs">1. Basic Information</h2>
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div class="sm:col-span-2">
                ...
                <input id="title" v-model="formData.title" type="text" required
                  placeholder="Brief description of the issue" class="input input-bordered w-full rounded-xl border-base-300 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-sans" />
              </div>
              ...
              <div class="sm:col-span-2">
                ...
                <textarea id="description" v-model="formData.description" required rows="4"
                  ...
                  class="textarea textarea-bordered w-full rounded-xl border-base-300 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-sans"></textarea>
              </div>
            </div>
          </div>
          ...
          <div class="p-6 md:p-8 bg-base-200/50">
            <h2 class="text-lg font-bold font-mono text-slate-200 mb-6 uppercase tracking-wider text-xs">2. Location Capture</h2>
            <LocationSelector v-model="formData.location" :auto-locate="true" />
          </div>

          <!-- Media -->
          <div class="p-6 md:p-8">
            <h2 class="text-lg font-bold font-mono text-slate-200 mb-6 uppercase tracking-wider text-xs">3. Attach Media Evidence</h2>

            <!-- Images -->
            <div class="mb-8">
              <label class="label"><span class="label-text font-mono text-xs text-slate-400 uppercase tracking-wider">Images (At least one required) *</span></label>
              ...
              <div v-if="imagePreviews.length > 0"
                ...
                <div v-for="(preview, index) in imagePreviews" :key="index" class="relative group aspect-square rounded-xl overflow-hidden border border-base-300">
            ...
            <!-- Voice Note -->
            <div class="mb-8">
              <label class="label"><span class="label-text font-mono text-xs text-slate-400 uppercase tracking-wider">Voice Note (optional)</span></label>
              ...
              <!-- Voice Note Preview -->
              <div v-if="formData.voiceNote" class="bg-base-100 border border-base-300 p-4 rounded-xl mt-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center font-mono text-xs text-slate-300">
                    <svg class="w-5 h-5 text-secondary mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round"
                        d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
                    </svg>
                    <span class="truncate">{{ formData.voiceNote.name }}</span>
                  </div>
                  <button type="button" @click="formData.voiceNote = null" class="text-error hover:text-red-400 cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <!-- Video Note -->
            <div>
              <label class="label"><span class="label-text font-mono text-xs text-slate-400 uppercase tracking-wider">Video Note (optional)</span></label>
              ...
              <!-- Video Note Preview -->
              <div v-if="formData.videoNote" class="bg-base-100 border border-base-300 p-4 rounded-xl mt-2">
                <div class="flex items-center justify-between">
                  <div class="flex items-center font-mono text-xs text-slate-300">
                    <svg class="w-5 h-5 text-primary mr-2" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round"
                        d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
                    </svg>
                    <span class="truncate">{{ formData.videoNote.name }}</span>
                  </div>
                  <button type="button" @click="formData.videoNote = null" class="text-error hover:text-red-400 cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                  </button>
          ...
          <div class="p-6 md:p-8 bg-base-200/80 sticky bottom-0 z-10 border-t border-base-300">
            <div class="flex flex-col sm:flex-row sm:justify-end gap-3">
              ...
              <button type="submit" :disabled="loading || !isFormValid" class="btn btn-primary rounded-xl cursor-pointer font-bold px-8 shadow-lg shadow-blue-500/10">
      ...
      <div v-if="error" class="border border-error/20 bg-error/5 text-error text-center text-xs font-mono p-4 rounded-xl mt-6">
        {{ error }}
      </div>
    </div>
