  ...
  <div class="min-h-screen bg-base-100 text-base-content antialiased py-8 px-4 sm:px-6 lg:px-8">
    ...
    <main class="max-w-4xl mx-auto">
      <div>
        <!-- Back button -->
        <div class="mb-8">
          <button @click="$router.go(-1)" class="btn btn-outline border-base-300 hover:border-slate-500 rounded-xl btn-sm font-mono flex items-center gap-1.5 cursor-pointer">
        ...
        <!-- Issue Details Card -->
        <div v-else-if="issue" class="bg-base-200 border border-base-300 shadow-xl rounded-3xl overflow-hidden">
          <!-- Header -->
          <div class="p-6 md:p-8 border-b border-base-300">
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div class="flex-1">
                <h1 class="text-2xl md:text-3xl font-extrabold text-slate-100">{{ issue.title }}</h1>
                <div class="mt-3 flex flex-wrap items-center gap-4 text-xs font-mono text-slate-400">
                  <div class="flex items-center">
                    <svg class="w-4 h-4 mr-1.5 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              ...
              <div class="flex items-center gap-3">
                ...
                <button @click="toggleUpvote"
                  :class="['btn btn-sm rounded-xl font-mono cursor-pointer', issue.user_upvoted ? 'btn-primary' : 'btn-outline border-base-300 hover:border-slate-500']">
          ...
          <div class="p-6 md:p-8">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Description</h3>
          ...
          <div v-if="(issue.issue_type && issue.issue_type !== 'Unspecified') || issue.address || issue.department"
            ...
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Details</h3>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
              <div v-if="issue.issue_type && issue.issue_type !== 'Unspecified'">
                <span class="text-2xs font-mono text-slate-500 uppercase tracking-widest block">Issue Type</span>
              ...
              <div v-if="issue.address">
                <span class="text-2xs font-mono text-slate-500 uppercase tracking-widest block">Address</span>
                <span class="mt-1.5 text-sm text-slate-200 flex items-center">
              ...
              <div v-if="issue.department">
                <span class="text-2xs font-mono text-slate-500 uppercase tracking-widest block">Assigned Department</span>
                <span class="mt-1.5 text-sm text-slate-200 flex items-center">
          ...
          <div v-if="aiVerification" class="border-t border-base-300 p-6 md:p-8 bg-base-200/50">
            <h3 class="text-xs font-bold font-mono uppercase tracking-wider text-slate-400 mb-4 flex items-center gap-2">
            ...
            <div :class="{
              'bg-emerald-500/10 border-emerald-500/20 text-emerald-400': aiVerification.status === 'verified',
              'bg-red-500/10 border-red-500/20 text-red-400': aiVerification.status === 'rejected',
              'bg-yellow-500/10 border-yellow-500/20 text-yellow-400': aiVerification.status === 'pending',
              ...
              <div class="flex items-center gap-2 mb-2">
                ...
                }" class="font-bold uppercase tracking-wider">{{ aiVerification.status }}</span>
          ...
          <div v-if="issue.latitude && issue.longitude" class="border-t border-base-300 p-6 md:p-8">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Location Map</h3>
          ...
          <div v-if="issue.image_urls?.length > 0 || issue.voice_note_url || issue.video_note_url"
            ...
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-6">Attached Media</h3>
            <div class="space-y-6">
              <div v-if="issue.image_urls && issue.image_urls.length > 0">
                <h4 class="text-xs font-mono font-bold text-slate-500 mb-3 flex items-center uppercase tracking-wider">
                  <svg class="w-4 h-4 mr-2 text-slate-500" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
              ...
              <div v-if="issue.video_note_url">
                ...
                <video controls :src="issue.video_note_url" class="w-full max-w-md rounded-2xl border border-base-300 shadow-md bg-black"></video>
              </div>
            </div>
          </div>
          ...
          <div class="border-t border-base-300 p-6 md:p-8">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Updates Feed</h3>
            <div v-if="updates.length === 0" class="text-sm font-mono text-slate-500">No updates uploaded yet.</div>
            <div v-else class="space-y-4">
              <div v-for="u in updates" :key="u.id" class="bg-base-100/50 border border-base-300 rounded-2xl p-5 shadow-inner">
                ...
                <div v-if="u.image_urls && u.image_urls.length > 0">
                  <h5 class="text-xs font-mono font-bold text-slate-500 mb-3 uppercase tracking-wider">Update Attachments</h5>
          ...
          <div class="border-t border-base-300 p-6 md:p-8 bg-base-200/40">
            <h3 class="font-mono text-xs font-bold uppercase tracking-wider text-slate-400 mb-4">Comments ({{ comments.length }})</h3>
            <div v-if="comments.length === 0" class="text-sm font-mono text-slate-500 mb-6">No public comments.</div>
            <div v-else class="space-y-4 mb-6">
              <div v-for="c in comments" :key="c.id" class="bg-base-100/40 border border-base-300 rounded-2xl p-4 shadow-sm">
                <div class="flex items-center justify-between mb-2 gap-2 text-xs font-mono text-slate-500">
                  <span class="font-bold text-slate-300">{{ c.author?.firstname }} {{ c.author?.lastname }}</span>
            ...
            <div class="flex flex-col sm:flex-row gap-3">
              <input v-model="newComment" type="text" placeholder="Add a comment..."
                class="input input-bordered flex-1 rounded-xl border-base-300 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-sans text-sm" @keyup.enter="submitComment" />
              <button @click="submitComment" :disabled="!newComment.trim()" class="btn btn-primary rounded-xl cursor-pointer font-bold px-6">Post Comment</button>
            </div>
          </div>
        </div>
        ...
        <div v-else class="text-center py-16 border border-dashed border-base-300 rounded-3xl bg-base-200/40">
          ...
          <p class="mt-1.5 text-sm text-slate-500 font-sans">The issue you're looking for doesn't exist or you don't have permission to view it.</p>
        </div>
      </div>
    </main>
