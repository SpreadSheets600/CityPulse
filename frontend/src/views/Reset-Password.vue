  ...
  <div class="min-h-screen flex items-center justify-center bg-base-100 p-6 relative overflow-hidden text-base-content antialiased">
    ...
    <div class="max-w-md w-full space-y-8 relative z-10">
      ...
      <div class="glass-panel shadow-2xl rounded-3xl p-8 md:p-10 border border-base-300 bg-base-200/55 backdrop-blur-lg">
        ...
        <div v-if="success" class="border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 p-4 rounded-xl text-center text-sm font-mono mb-4">
          Password Reset Successfully. <router-link to="/login" class="text-primary hover:text-blue-400 underline ml-1">Sign in</router-link>
        </div>

        <form v-else class="space-y-5" @submit.prevent="handleResetPassword">
          <div>
            <label for="token" class="label"><span class="label-text font-mono text-xs text-slate-400 uppercase tracking-wider">Reset Token</span></label>
            <input id="token" type="text" required class="input input-bordered w-full rounded-xl border-base-300 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-sans"
              placeholder="Paste your reset token" v-model="form.token" />
          </div>
          <div>
            <label for="password" class="label"><span class="label-text font-mono text-xs text-slate-400 uppercase tracking-wider">New Password</span></label>
            <input id="password" type="password" required minlength="8" class="input input-bordered w-full rounded-xl border-base-300 focus:border-primary focus:ring-1 focus:ring-primary transition-all font-sans"
