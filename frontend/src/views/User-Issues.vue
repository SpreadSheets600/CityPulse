    ...
    <main class="max-w-7xl mx-auto">
      <div>
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
          <div>
            <h2 class="text-3xl font-extrabold text-slate-100 font-mono tracking-wider uppercase">My Reported Issues</h2>
            <p class="text-sm text-slate-400 mt-1">Track updates and SLA responses for your submissions</p>
          </div>
        ...
        <!-- Issues list -->
        <div v-else-if="issues.length > 0" class="bg-base-200 border border-base-300 shadow-xl rounded-3xl overflow-hidden">
          <ul class="divide-y divide-base-300">
            <li v-for="issue in issues" :key="issue.id" class="hover:bg-base-300/10 transition-colors">
              <router-link :to="`/issues/${issue.id}`" class="block p-5 sm:px-8">
                ...
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 gap-2">
                  <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1 line-clamp-2 leading-relaxed font-sans">{{ issue.description }}</p>
                  </div>
                ...
                <!-- Details Row -->
                <div class="flex flex-wrap items-center gap-3 text-xs font-mono text-slate-500 pt-1">
                ...
                <!-- Footer updates -->
                <div v-if="issue.updated_at !== issue.created_at" class="flex items-center text-xs font-mono text-slate-500 mt-2.5 pt-2.5 border-t border-base-300/30">
        ...
        <div v-else class="text-center py-16 border border-dashed border-base-300 rounded-3xl bg-base-200/40">
          ...
          <p class="mt-1.5 text-sm text-slate-500">Get started by reporting your first neighborhood issue.</p>
          <div class="mt-6">
            <router-link to="/issues/create"
