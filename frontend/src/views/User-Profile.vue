<template>
  <div class="min-h-screen bg-base-100 text-base-content antialiased py-8 px-4 sm:px-6 lg:px-8">
    <main class="max-w-4xl mx-auto">
      <div>
        <div class="mb-8">
          <h2 class="text-3xl font-extrabold text-slate-100 font-mono tracking-wider uppercase">User Profile</h2>
          <p class="mt-1 text-sm text-slate-400">Manage your credentials and neighborhood association profile.</p>
        </div>

        <div class="bg-base-200 border border-base-300 shadow-xl rounded-3xl overflow-hidden p-6 md:p-8">
          ...
          <div>
            <div class="flex items-center mb-8 pb-8 border-b border-base-300">
              ...
              <div class="ml-5">
                <h3 class="text-xl font-bold text-slate-100">{{ user?.firstname }} {{ user?.lastname }}</h3>
                <p class="text-sm font-mono text-slate-400 mt-1">{{ user?.email }}</p>
              </div>
            </div>

            <!-- Notifications -->
            <div v-if="success" class="border border-emerald-500/20 bg-emerald-500/5 text-emerald-400 p-4 rounded-xl text-center text-xs font-mono mb-6">
              Profile Updated Successfully
            </div>
            <div v-if="error" class="border border-error/20 bg-error/5 text-error text-center text-xs font-mono p-4 rounded-xl mb-6">
              {{ error }}
            </div>

            <div class="flex justify-between items-center mb-6">
              <h3 class="text-xs font-bold font-mono text-slate-400 uppercase tracking-widest">Personal Information</h3>
              <button v-if="!editing" @click="startEditing"
                class="btn btn-sm btn-outline border-base-300 hover:border-slate-500 rounded-xl font-mono cursor-pointer">Edit</button>
              <div v-else class="flex gap-2">
                <button @click="saveProfile" :disabled="saving" class="btn btn-sm btn-primary rounded-xl font-mono cursor-pointer">
                  {{ saving ? 'SAVING...' : 'SAVE CHANGES' }}
                </button>
                <button @click="cancelEditing" class="btn btn-sm btn-ghost rounded-xl font-mono cursor-pointer">Cancel</button>
              </div>
            </div>

            <dl class="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-2">
              <div class="sm:col-span-1">
                <dt class="text-2xs font-mono text-slate-500 uppercase tracking-widest">First Name</dt>
              ...
              <div class="sm:col-span-1">
                <dt class="text-2xs font-mono text-slate-500 uppercase tracking-widest">Last Name</dt>
              ...
              <div class="sm:col-span-1">
                ...
                <dd v-if="!editing" class="mt-1 text-sm text-slate-200 font-mono">{{ user?.phone }}</dd>
              ...
              <div class="sm:col-span-1">
                <dt class="text-2xs font-mono text-slate-500 uppercase tracking-widest">Member Since</dt>
                <dd class="mt-1.5 text-sm text-slate-200 font-mono">{{ formatDate(user?.created_at) }}</dd>
              </div>
            </dl>
          </div>
        </div>
